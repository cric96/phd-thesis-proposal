...
As target toolkit, we will adopt \scafi{} in my thesis work mostly for practical reasons: concerning other aggregate programming languages such as Proto and Protelis, surveyed in \cite{viroli2019jlamp-si-coord},
...
its abstract space and its application in concrete systems. In this direction, I made mainly contributions to \scafi. In particular, we produce two articles concerning ScaFi-Web (described in \Cref{scafi-web} ) -- a web-based tool that could support monitoring aspects and \scafi Loci (described in \Cref{scalaloci}) -- a type-safe deployment methodology leveraging multi-tier programming.
\subsubsection{ScaFi-Web -- A tool for a distributed monitoring} \label{scafi-web}
...
\subsubsection{ScaFi Loci -- Towards a type-safe deployment of Pulverised Architecture}\label{scalaloci}
...
\end{itemize}
\subsection{Reinforcement Learning applied to Building Blocks}
...
Our hypothesis in~\cite{research} consists of applied Machine Learning algorithms -- and in particular Reinforcement Learning -- to improve building blocks used by speeding up the self-stabilisation process.
...
\begin{itemize}
	\item publishing our first results leveraging tabular methodologies -- thi.s will be a baseline;
...
Novel researches tackle this problem, by maintaining the same high-level specification (i.e. the \emph{aggregate program}) but by fine-tuning underlying aspects (e.g. round frequency).
%%
The forthcoming works aim to concretise a middleware capable of supporting computation \emph{flexibly} and \emph{intelligently}.
