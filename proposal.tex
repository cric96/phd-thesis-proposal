...
%% Aggregate computing give a theoretical framework by which is possible to describe composable collective behaviours
%% It abstract over various aspect such us aggregate program evaluation, neighoborhood discovery, and message sending.
%% Novel research try to focusing on such aspect.
%% In particular, distributed scheduler is a novel techinquie by which is possbile to define when the program will be locally execyuted, using the same  aggregate computing language
%% Speaking about message sending, when we have to deal with high density network, it problably have sense to reduce the message sent to neighbours to avoid  a message waste.
%% Talk about the aspect of time field (distributed scheduler) and space fluid (discard message opportunistically).
