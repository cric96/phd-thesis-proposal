...
\abstract{
	...
	In particular, I plan to leverage \ac{ac} -- a novel top-down global-to-local programming model.
	%
...
The nodes could be heterogeneous, but I imagine a \emph{homogenous} local behaviour (i.e., each node should execute the same local logic). 
%
...
%
Even if modern architectures could be used (e.g cloud/edge infrastructures), in general, the systems needs to react to local problems quickly. 
%
...
%
I aim to expand this vision also to other ``swarm-like'' systems, such as a crowd of people, large-scale IoT and smart cities where we do not have robots. 
...
%
Finally, in the \ac{ac} literature, we typically leverage simulations to verify collective behaviours since it is costly to set up a large-scale bench test. 
%
...
%
I have adopted \scafi{} mostly for practical reasons: concerning other aggregate programming languages such as Proto and Protelis, surveyed in \cite{viroli2019jlamp-si-coord},
...
Currently, the problem with those building blocks is that they do not work well in any network topology. In literature, indeed, when we are dealing with high node dynamic, the overall computation field becomes quite noised and unstable -- leading to not reaching our intended result.

Traditionally, these problems are tackled with heuristics/algorithms that solve a particular issue (stability of results, convergence speed up, \dots{}). However, this leads to complex parameters fine-tuning in each new environment.
